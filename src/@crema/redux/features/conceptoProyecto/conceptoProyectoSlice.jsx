// src/redux/features/conceptos-proyectos/conceptos-proyectosSlice.js
...
export const onGetColeccionLigera = createAsyncThunk(
  'conceptoProyecto/onGetColeccionLigera',
  async (_, thunkAPI) => {
    try {
      const response = await jwtAxios.get('conceptos-proyectos', {
        params: { ligera: true },
...
export const onShow = createAsyncThunk(
  'conceptoProyecto/onShow',
  async (id, thunkAPI) => {
    try {
      const response = await jwtAxios.get(`conceptos-proyectos/${id}`);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(messages['message.somethingWentWrong'] || error.message);
    }
...
export const onUpdate = createAsyncThunk(
  'conceptoProyecto/onUpdate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.put(`conceptos-proyectos/${params.id}`, params);
...
export const onDelete = createAsyncThunk(
  'conceptoProyecto/onDelete',
  async ({ id, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.delete(`conceptos-proyectos/${id}`);
      updateColeccion();
      const mensaje = response.data.mensajes[0];
      const tipoMensaje = response.data.mensajes[1];
...
export const onCreate = createAsyncThunk(
  'conceptoProyecto/onCreate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
   
    try {
      const response = await jwtAxios.post('conceptos-proyectos', params);
      const mensaje = response.data.mensajes[0];
      const tipoMensaje = response.data.mensajes[1];
      thunkAPI.dispatch(showMessage([mensaje, tipoMensaje]));
      if(tipoMensaje != 4){
...
// Slice de conceptos-proyectos
const conceptoProyectoSlice = createSlice({
  name: 'conceptoProyecto',
  initialState,
  reducers: {
    resetError(state) {
      state.error = null;
    },
    resetConceptoProyectoActual(state) {
      state.ConceptoProyectoActual = null;
    },
    resetMessage(state) {
      state.message = null;
    },
  ...
  extraReducers: (builder) => {
    builder
      // Obtener colección de conceptos-proyectos
      ...
      // Obtener colección ligera de conceptos-proyectos
      ...
      .addCase(onShow.fulfilled, (state, action) => {
        state.loading = false;
        state.ConceptoProyectoActual = action.payload;
      })
      ...
      // Actualizar un ConceptoProyecto
...
export const { resetError, resetConceptoProyectoActual, resetMessage } = conceptoProyectoSlice.actions;
export default conceptoProyectoSlice.reducer;
