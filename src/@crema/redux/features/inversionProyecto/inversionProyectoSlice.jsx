// src/redux/features/inversiones-proyectos/inversiones-proyectosSlice.js
...
export const onGetColeccion = createAsyncThunk(
    'inversiones-proyectos/onGetColeccion',
    async ({page,rowsPerPage,inversion_id, idInversionista, idProyecto, estadoFiltro, fechaDesdeFiltro, fechaHastaFiltro, orderByToSend}, thunkAPI) => {
      try {
        page = page ? page : 0;
        const inversionIdAux = inversion_id ? inversion_id : '';
        const nombreAux = idInversionista ? idInversionista : '';
        const estadoAux = estadoFiltro ? estadoFiltro : '';
        const proyectoAux = idProyecto ? idProyecto : '';
        const fechaDesdeAux = fechaDesdeFiltro ? fechaDesdeFiltro : '';
        const fechaHastaAux = fechaHastaFiltro ? fechaHastaFiltro : '';
        const ordenar_por = orderByToSend ? orderByToSend : '';
        const response = await jwtAxios.get('inversiones-proyectos', {
          params: {
            page: page,
            limite: rowsPerPage,
            id_inversion: inversionIdAux,
            id_inversionista: nombreAux,
            estado_inversion: estadoAux,
            id_proyecto: proyectoAux,
...
export const onGetColeccionLigera = createAsyncThunk(
  'inversiones-proyectos/onGetColeccionLigera',
  async (_, thunkAPI) => {
    try {
      const response = await jwtAxios.get('inversiones-proyectos', {
        params: { ligera: true },
...
export const onShow = createAsyncThunk(
  'inversiones-proyectos/onShow',
  async (id, thunkAPI) => {
    try {
      const response = await jwtAxios.get(`inversiones-proyectos/${id}`);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(messages['message.somethingWentWrong'] || error.message);
    }
...
export const onUpdate = createAsyncThunk(
  'inversiones-proyectos/onUpdate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.put(`inversiones-proyectos/${params.id}`, params);
...
export const onDelete = createAsyncThunk(
  'inversiones-proyectos/onDelete',
  async ({ id, updateColeccion }, thunkAPI) => {
    console.log(id)
    try {
      const response = await jwtAxios.delete(`inversiones-proyectos/${id}`);
      updateColeccion();
...
export const onCreate = createAsyncThunk(
  'inversiones-proyectos/onCreate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.post('inversiones-proyectos', params);
      handleOnClose();
      updateColeccion();
...
// Slice de inversiones-proyectos
const InversionProyectoSlice = createSlice({
  name: 'inversiones-proyectos',
  initialState,
  reducers: {
    resetError(state) {
      state.error = null;
    },
    resetInversionProyectoActual(state) {
      state.InversionProyectoActual = null;
    },
    resetMessage(state) {
      state.message = null;
    },
  ...
  extraReducers: (builder) => {
    builder
      // Obtener colección de inversiones-proyectos
      ...
      // Obtener colección ligera de inversiones-proyectos
      ...
      .addCase(onShow.fulfilled, (state, action) => {
        state.loading = false;
        state.InversionProyectoActual = action.payload;
      })
      ...
      // Actualizar un InversionProyecto
...
export const { resetInversionProyectoActual } = InversionProyectoSlice.actions;
export default InversionProyectoSlice.reducer;
