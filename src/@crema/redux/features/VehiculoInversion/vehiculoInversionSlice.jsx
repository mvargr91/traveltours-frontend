// src/redux/features/vehiculos-inversion/vehiculos-inversionSlice.js
...
export const onGetColeccionLigera = createAsyncThunk(
  'vehiculosInversion/onGetColeccionLigera',
  async (_, thunkAPI) => {
    try {
      const response = await jwtAxios.get('vehiculos-inversion', {
        params: { ligera: true },
...
export const onShow = createAsyncThunk(
  'vehiculosInversion/onShow',
  async (id, thunkAPI) => {
    try {
      const response = await jwtAxios.get(`vehiculos-inversion/${id}`);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(messages['message.somethingWentWrong'] || error.message);
    }
...
export const onUpdate = createAsyncThunk(
  'vehiculosInversion/onUpdate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.put(`vehiculos-inversion/${params.id}`, params);
...
export const onDelete = createAsyncThunk(
  'vehiculosInversion/onDelete',
  async ({ id, updateColeccion }, thunkAPI) => {
    console.log(id)
    try {
      const response = await jwtAxios.delete(`vehiculos-inversion/${id}`);
      updateColeccion();
...
export const onCreate = createAsyncThunk(
  'vehiculosInversion/onCreate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.post('vehiculos-inversion', params);
      handleOnClose();
      updateColeccion();
...
// Slice de vehiculos-inversion
const vehiculosInversionSlice = createSlice({
  name: 'vehiculosInversion',
  initialState,
  reducers: {
    resetError(state) {
      state.error = null;
    },
    resetVehiculoInversionActual(state) {
      state.VehiculoInversionActual = null;
    },
    resetMessage(state) {
      state.message = null;
    },
  ...
  extraReducers: (builder) => {
    builder
      // Obtener colección de vehiculos-inversion
      ...
      // Obtener colección ligera de vehiculos-inversion
      ...
      .addCase(onShow.fulfilled, (state, action) => {
        state.loading = false;
        state.VehiculoInversionActual = action.payload;
      })
      ...
      // Actualizar un VehiculoInversion
...
export const { resetError, resetVehiculoInversionActual, resetMessage } = vehiculosInversionSlice.actions;
export default vehiculosInversionSlice.reducer;
