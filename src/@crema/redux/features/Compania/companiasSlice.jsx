// src/redux/features/companias/companiasSlice.js
...
export const onGetColeccionLigera = createAsyncThunk(
  'companias/onGetColeccionLigera',
  async (_, thunkAPI) => {
    try {
      const response = await jwtAxios.get('companias', {
        params: { ligera: true },
...
export const onShow = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await jwtAxios.get(`companias/${id}`);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(messages['message.somethingWentWrong'] || error.message);
    }
...
export const onUpdate = createAsyncThunk(
  ...
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.put(`companias/${params.id}`, params);
...
export const onDelete = createAsyncThunk(
  ...
  async ({ id, updateColeccion }, thunkAPI) => {
    console.log(id)
    try {
      const response = await jwtAxios.delete(`companias/${id}`);
      updateColeccion();
...
export const onCreate = createAsyncThunk(
  ...
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.post('companias', params);
      handleOnClose();
      updateColeccion();
...
const companiasSlice = createSlice({
  ...
  reducers: {
    ...
    resetCompaniaActual(state) {
      state.CompaniaActual = null;
    },
    resetMessage(state) {
      state.message = null;
    },
  ...
  extraReducers: (builder) => {
    builder
      // Obtener colección de companias
      ...
      // Obtener colección ligera de companias
      ...
      .addCase(onShow.fulfilled, (state, action) => {
        state.loading = false;
        state.CompaniaActual = action.payload;
      })
...
export const { resetError, resetCompaniaActual, resetMessage } = companiasSlice.actions;
