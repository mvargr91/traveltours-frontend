// src/redux/features/precios-proyectados/precios-proyectadosSlice.js
...
export const onGetColeccion = createAsyncThunk(
    'precioProyectado/onGetColeccion',
    async ({page, rowsPerPage, anioFiltro, orderByToSend}, thunkAPI) => {
      try {
        page = page ? page : 0;
        const anioAux = anioFiltro ? anioFiltro : '';
        const ordenar_por = orderByToSend ? orderByToSend : '';
        const response = await jwtAxios.get('precios-proyectados', {
          params: {
            page: page,
            limite: rowsPerPage,
            anio: anioAux,
...
export const onGetColeccionLigera = createAsyncThunk(
  'precioProyectado/onGetColeccionLigera',
  async (_, thunkAPI) => {
    try {
      const response = await jwtAxios.get('precios-proyectados', {
        params: { ligera: true },
...
export const onShow = createAsyncThunk(
  'precioProyectado/onShow',
  async (id, thunkAPI) => {
    try {
      const response = await jwtAxios.get(`precios-proyectados/${id}`);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(messages['message.somethingWentWrong'] || error.message);
    }
...
export const onUpdate = createAsyncThunk(
  'precioProyectado/onUpdate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.put(`precios-proyectados/${params.id}`, params);
...
export const onDelete = createAsyncThunk(
  'precioProyectado/onDelete',
  async ({ id, updateColeccion }, thunkAPI) => {
    console.log(id)
    try {
      const response = await jwtAxios.delete(`precios-proyectados/${id}`);
      updateColeccion();
...
export const onCreate = createAsyncThunk(
  'precioProyectado/onCreate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.post('precios-proyectados', params);
      handleOnClose();
      updateColeccion();
...
// Slice de precios-proyectados
const precioProyectadoSlice = createSlice({
  name: 'precioProyectado',
  initialState,
  reducers: {
    resetError(state) {
      state.error = null;
    },
    resetPrecioProyectadoActual(state) {
      state.PrecioProyectadoActual = null;
    },
    resetMessage(state) {
      state.message = null;
    },
  ...
  extraReducers: (builder) => {
    builder
      // Obtener colección de precios-proyectados
      ...
      // Obtener colección ligera de precios-proyectados
      ...
      .addCase(onShow.fulfilled, (state, action) => {
        state.loading = false;
        state.PrecioProyectadoActual = action.payload;
      })
      ...
      // Actualizar un PrecioProyectado
...
export const { resetError, resetPrecioProyectadoActual, resetMessage } = precioProyectadoSlice.actions;
export default precioProyectadoSlice.reducer;
