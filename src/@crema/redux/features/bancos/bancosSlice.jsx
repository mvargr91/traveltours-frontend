// src/redux/features/bancos/bancosSlice.js
...
export const onShow = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await jwtAxios.get(`bancos/${id}`);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(messages['message.somethingWentWrong'] || error.message);
    }
...
export const onUpdate = createAsyncThunk(
  ...
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.put(`bancos/${params.id}`, params);
...
export const onDelete = createAsyncThunk(
  ...
  async ({ id, updateColeccion }, thunkAPI) => {
    console.log(id)
    try {
      const response = await jwtAxios.delete(`bancos/${id}`);
      updateColeccion();
...
export const onCreate = createAsyncThunk(
  ...
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.post('bancos', params);
      handleOnClose();
      updateColeccion();
...
const bancosSlice = createSlice({
  ...
  reducers: {
    ...
    resetBancoActual(state) {
      state.bancoActual = null;
    },
    resetMessage(state) {
      state.message = null;
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Obtener colección ligera de bancos
...
export const { resetError, resetBancoActual, resetMessage } = bancosSlice.actions;
