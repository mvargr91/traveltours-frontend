// src/redux/features/tipos-inversiones/tipos-inversionesSlice.js
...
export const onGetColeccionLigera = createAsyncThunk(
  'categoriaInversiones/onGetColeccionLigera',
  async (_, thunkAPI) => {
    try {
      const response = await jwtAxios.get('tipos-inversiones', {
        params: { ligera: true },
...
export const onShow = createAsyncThunk(
  'categoriaInversiones/onShow',
  async (id, thunkAPI) => {
    try {
      const response = await jwtAxios.get(`tipos-inversiones/${id}`);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(messages['message.somethingWentWrong'] || error.message);
    }
...
export const onUpdate = createAsyncThunk(
  'categoriaInversiones/onUpdate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.put(`tipos-inversiones/${params.id}`, params);
...
export const onDelete = createAsyncThunk(
  'categoriaInversiones/onDelete',
  async ({ id, updateColeccion }, thunkAPI) => {
    console.log(id)
    try {
      const response = await jwtAxios.delete(`tipos-inversiones/${id}`);
      updateColeccion();
...
export const onCreate = createAsyncThunk(
  'categoriaInversiones/onCreate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.post('tipos-inversiones', params);
      handleOnClose();
      updateColeccion();
...
// Slice de tipos-inversiones
const categoriaInversionesSlice = createSlice({
  name: 'tipos-inversiones',
  initialState,
  reducers: {
    resetError(state) {
      state.error = null;
    },
    resetCategoriaInversionActual(state) {
      state.CategoriaInversionActual = null;
    },
    resetMessage(state) {
      state.message = null;
    },
  ...
  extraReducers: (builder) => {
    builder
      // Obtener colección de tipos-inversiones
      ...
      // Obtener colección ligera de tipos-inversiones
      ...
      .addCase(onShow.fulfilled, (state, action) => {
        state.loading = false;
        state.CategoriaInversionActual = action.payload;
      })
      ...
      // Actualizar un CategoriaInversion
      ...
      // Eliminar un CategoriaInversion
...
export const { resetError, resetCategoriaInversionActual, resetMessage } = categoriaInversionesSlice.actions;
export default categoriaInversionesSlice.reducer;
