// src/redux/features/gestores/gestoresSlice.js
...
export const onShow = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await jwtAxios.get(`gestores/${id}`);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(messages['message.somethingWentWrong'] || error.message);
    }
...
export const onUpdate = createAsyncThunk(
  ...
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.put(`gestores/${params.id}`, params);
...
export const onDelete = createAsyncThunk(
  ...
  async ({ id, updateColeccion }, thunkAPI) => {
    console.log(id)
    try {
      const response = await jwtAxios.delete(`gestores/${id}`);
      updateColeccion();
...
export const onCreate = createAsyncThunk(
  ...
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.post('gestores', params);
      handleOnClose();
      updateColeccion();
...
const gestoresSlice = createSlice({
  ...
  reducers: {
    ...
    resetGestorActual(state) {
      state.GestorActual = null;
    },
    resetMessage(state) {
      state.message = null;
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Obtener colección ligera de gestores
      ...
      .addCase(onShow.fulfilled, (state, action) => {
        state.loading = false;
        state.GestorActual = action.payload;
      })
...
export const { resetError, resetGestorActual, resetMessage } = gestoresSlice.actions;
