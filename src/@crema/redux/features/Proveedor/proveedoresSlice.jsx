// src/redux/features/proveedores/proveedoresSlice.js
...
export const onGetColeccionLigera = createAsyncThunk(
  'proveedores/onGetColeccionLigera',
  async (_, thunkAPI) => {
    try {
      const response = await jwtAxios.get('proveedores', {
        params: { ligera: true },
...
export const onShow = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await jwtAxios.get(`proveedores/${id}`);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(messages['message.somethingWentWrong'] || error.message);
    }
...
export const onUpdate = createAsyncThunk(
  'proveedores/onUpdate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.put(`proveedores/${params.id}`, params);
...
export const onDelete = createAsyncThunk(
  'proveedores/onDelete',
  async ({ id, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.delete(`proveedores/${id}`);
      updateColeccion();
...
export const onCreate = createAsyncThunk(
  'proveedores/onCreate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.post('proveedores', params);
      handleOnClose();
      updateColeccion();
...
const proveedoresSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Obtener colección ligera de proveedores
...
export const { resetError, resetProveedorActual, resetMessage } = proveedoresSlice.actions;
