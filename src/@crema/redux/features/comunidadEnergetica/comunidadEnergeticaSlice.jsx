// src/redux/features/comunidadEnergetica/comunidadEnergeticaSlice.js
...
const initialState = {
  ...
  total: 0,
  coleccionLigera: [],
  comunidadEnergeticaActual: null,
  loading: false,
  error: null,
  message: null,
};
...
export const onGetColeccionLigera = createAsyncThunk(
  'comunidadEnergetica/onGetColeccionLigera',
  async (_, thunkAPI) => {
    try {
      const response = await jwtAxios.get('comunidades-energeticas', {
        params: { ligera: true },
...
export const onShow = createAsyncThunk(
  'comunidadEnergetica/onShow',
  async (id, thunkAPI) => {
    try {
      const response = await jwtAxios.get(`comunidades-energeticas/${id}`);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(messages['message.somethingWentWrong'] || error.message);
    }
...
export const onUpdate = createAsyncThunk(
  'comunidadEnergetica/onUpdate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.put(`comunidades-energeticas/${params.id}`, params);
...
export const onDelete = createAsyncThunk(
  'comunidadEnergetica/onDelete',
  async ({ id, updateColeccion }, thunkAPI) => {
    console.log(id)
    try {
      const response = await jwtAxios.delete(`comunidades-energeticas/${id}`);
      updateColeccion();
...
export const onCreate = createAsyncThunk(
  'comunidadEnergetica/onCreate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.post('comunidades-energeticas', params);
      handleOnClose();
      updateColeccion();
...
// Slice de comunidadEnergetica
const comunidadEnergeticaSlice = createSlice({
  ...
  reducers: {
    ...
    resetComunidadEnergeticaActual(state) {
      state.comunidadEnergeticaActual = null;
    },
    resetMessage(state) {
      state.message = null;
    },
  ...
  extraReducers: (builder) => {
    builder
      // Obtener colección de comunidadEnergetica
      ...
      // Obtener colección ligera de comunidadEnergetica
      ...
      // Mostrar un comunidadEnergetica
      ...
      .addCase(onShow.fulfilled, (state, action) => {
        state.loading = false;
        state.comunidadEnergeticaActual = action.payload;
      })
      ...
      // Actualizar un comunidadEnergetica
      ...
      // Eliminar un comunidadEnergetica
...
export const { resetError, resetComunidadEnergeticaActual, resetMessage } = comunidadEnergeticaSlice.actions;
export default comunidadEnergeticaSlice.reducer;
