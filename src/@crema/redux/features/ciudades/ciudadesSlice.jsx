// src/redux/features/ciudades/ciudadesSlice.js
...
export const onShow = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await jwtAxios.get(`ciudades/${id}`);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(messages['message.somethingWentWrong'] || error.message);
    }
...
export const onUpdate = createAsyncThunk(
  ...
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.put(`ciudades/${params.id}`, params);
...
export const onDelete = createAsyncThunk(
  ...
  async ({ id, updateColeccion }, thunkAPI) => {
    console.log(id)
    try {
      const response = await jwtAxios.delete(`ciudades/${id}`);
      updateColeccion();
...
export const onCreate = createAsyncThunk(
  ...
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.post('ciudades', params);
      handleOnClose();
      updateColeccion();
...
const ciudadesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Obtener colección ligera de ciudades
...
export const { resetError, resetCiudadActual, resetMessage } = ciudadesSlice.actions;
