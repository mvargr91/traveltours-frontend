// src/redux/features/tipos-proyectos/tiposProyectosSlice.js
...
export const onGetColeccionLigera = createAsyncThunk(
  'tiposProyectos/onGetColeccionLigera',
  async (_, thunkAPI) => {
    try {
      const response = await jwtAxios.get('tipos-proyectos', {
        params: { ligera: true },
...
export const onShow = createAsyncThunk(
  'tiposProyectos/onShow',
  async (id, thunkAPI) => {
    try {
      const response = await jwtAxios.get(`tipos-proyectos/${id}`);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(messages['message.somethingWentWrong'] || error.message);
    }
...
export const onUpdate = createAsyncThunk(
  'tiposProyectos/onUpdate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.put(`tipos-proyectos/${params.id}`, params);
...
export const onDelete = createAsyncThunk(
  'tiposProyectos/onDelete',
  async ({ id, updateColeccion }, thunkAPI) => {
    console.log(id)
    try {
      const response = await jwtAxios.delete(`tipos-proyectos/${id}`);
      updateColeccion();
...
export const onCreate = createAsyncThunk(
  'tiposProyectos/onCreate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.post('tipos-proyectos', params);
      handleOnClose();
      updateColeccion();
...
// Slice de tiposProyectos
const tiposProyectosSlice = createSlice({
  name: 'tiposProyectos',
  initialState,
  reducers: {
    resetError(state) {
    ...
    resetTipoProyectoActual(state) {
      state.tipoProyectoActual = null;
    },
    resetMessage(state) {
      state.message = null;
    },
  ...
  extraReducers: (builder) => {
    builder
      // Obtener colección de tiposProyectos
      ...
      // Obtener colección ligera de tiposProyectos
      ...
      .addCase(onShow.fulfilled, (state, action) => {
        state.loading = false;
        state.tipoProyectoActual = action.payload;
      })
...
export const { resetError, resetTipoProyectoActual, resetMessage } = tiposProyectosSlice.actions;
export default tiposProyectosSlice.reducer;
