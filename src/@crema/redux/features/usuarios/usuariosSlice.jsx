// src/redux/features/usuarios/usuariosSlice.js
...
export const onShow = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await jwtAxios.get(`usuarios/${id}`);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(messages['message.somethingWentWrong'] || error.message);
    }
...
export const onUpdate = createAsyncThunk(
  ...
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.put(`usuarios/${params.id}`, params);
...
export const onDelete = createAsyncThunk(
  ...
  async ({ id, updateColeccion }, thunkAPI) => {
    console.log(id)
    try {
      const response = await jwtAxios.delete(`usuarios/${id}`);
      updateColeccion();
...
export const onCreate = createAsyncThunk(
  ...
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.post('usuarios', params);
      handleOnClose();
      updateColeccion();
...
export const onChangePassword = createAsyncThunk(
  'usuarios/onChangePassword',
  async ({ params, handleOnClose  }, thunkAPI) => {
    try {
     
      const response = await jwtAxios.put('usuarios/cambio-clave', params);
      if (response.status === 200) {
        // Verifica si `handleOnClose` existe antes de ejecutarlo
        if (handleOnClose && typeof handleOnClose === 'function') {
          handleOnClose();
        }
        ...
        // En caso de error, despacha la acción de error
        const mensajeError = error.response?.data?.mensajes[0] || "Ocurrió un error";
        thunkAPI.dispatch(showMessage([response.data.mensajes[0], 4]));
        return thunkAPI.rejectWithValue(mensajeError);
      }
    } catch (error) {
...
const usuariosSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Obtener colección ligera de usuarios
      ...
      // Cambiar contraseña
      .addCase(onChangePassword.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(onChangePassword.fulfilled, (state) => {
        state.loading = false;
        state.error = null;
      })
...
export const { resetError, resetUsuarioActual, resetMessage } = usuariosSlice.actions;
