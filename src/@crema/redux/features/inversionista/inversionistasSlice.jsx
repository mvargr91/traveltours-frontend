// src/redux/features/inversionistas/inversionistasSlice.js
...
export const onGetColeccionLigera = createAsyncThunk(
  'inversionistas/onGetColeccionLigera',
  async (_, thunkAPI) => {
    try {
      const response = await jwtAxios.get('inversionistas', {
        params: { ligera: true },
...
export const onShow = createAsyncThunk(
  'inversionistas/onShow',
  async (id, thunkAPI) => {
    try {
      const response = await jwtAxios.get(`inversionistas/${id}`);
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(messages['message.somethingWentWrong'] || error.message);
    }
...
export const onUpdate = createAsyncThunk(
  'inversionistas/onUpdate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.put(`inversionistas/${params.id}`, params);
...
export const onDelete = createAsyncThunk(
  'inversionistas/onDelete',
  async ({ id, updateColeccion }, thunkAPI) => {
    console.log(id)
    try {
      const response = await jwtAxios.delete(`inversionistas/${id}`);
      updateColeccion();
...
export const onCreate = createAsyncThunk(
  'inversionistas/onCreate',
  async ({ params, handleOnClose, updateColeccion }, thunkAPI) => {
    try {
      const response = await jwtAxios.post('inversionistas', params);
      handleOnClose();
      updateColeccion();
...
// Slice de inversionistas
const inversionistasSlice = createSlice({
  name: 'inversionistas',
  initialState,
  reducers: {
    resetError(state) {
    ...
    resetInversionistaActual(state) {
      state.InversionistaActual = null;
    },
    resetMessage(state) {
      state.message = null;
    },
  ...
  extraReducers: (builder) => {
    builder
      // Obtener colección de inversionistas
      ...
      // Obtener colección ligera de inversionistas
      ...
      .addCase(onShow.fulfilled, (state, action) => {
        state.loading = false;
        state.InversionistaActual = action.payload;
      })
...
export const { resetError, resetInversionistaActual, resetMessage } = inversionistasSlice.actions;
export default inversionistasSlice.reducer;
