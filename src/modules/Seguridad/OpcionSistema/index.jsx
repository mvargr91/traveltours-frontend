...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddOpcionSistema: PropTypes.func.isRequired,
...
const OpcionSistema = (props) => {
  ...
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.opcionSistema);
  ...
  const debouncedModule = useDebounce(moduloFiltro, 800);
  ...
  useEffect(() => {
    setPage(1);
  }, [debouncedName, debouncedModule, orderByToSend]);
  ...
  const onOpenEditOpcionSistema = (id) => {
    setOpcionSistemaSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewOpcionSistema = (id) => {
    setOpcionSistemaSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteOpcionSistema = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar La Opción?',
