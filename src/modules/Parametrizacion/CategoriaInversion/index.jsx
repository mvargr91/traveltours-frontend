...
import CategoriaInversionCreador from './CategoriaInversionCreador';
import {onGetColeccion, onDelete} from '../../../@crema/redux/features/categoriaInversion/categoriaInversionesSlice';
...
import { formatCurrencyDecimal } from '../../../shared/hooks/formatCurrency';
...
const cells = [
  ...
  {
    id: 'tasa_interes_inversion',
    typeHead: 'numeric',
    label: 'Tasa interés (E.M)',
    value: (value) => formatCurrencyDecimal(value) + ' %',
    align: 'right',
    mostrarInicio: true,
  },
  {
    id: 'indicativo_forma_pago_int',
    typeHead: 'string',
    label: 'Forma pago',
    value: (value) => FORMA_PAGO.map((estado) => estado.id === value ? estado.nombre : ''),
    align: 'left',
    mostrarInicio: false,
  },
  {
    id: 'indicativo_asegurado',
    typeHead: 'string',
    label: 'Asegurado',
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear CategoriaInversion' onClick={onOpenAddCategoriaInversion}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddCategoriaInversion: PropTypes.func.isRequired,
...
const CategoriaInversion = (props) => {
  ...
  const [CategoriaInversionSeleccionado, setCategoriaInversionSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.categoriasInversiones);
  ...
  const onOpenEditCategoriaInversion = (id) => {
    setCategoriaInversionSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewCategoriaInversion = (id) => {
    setCategoriaInversionSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteCategoriaInversion = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar el tipo de inversión ?',
  ...
  const onOpenAddCategoriaInversion = () => {
    setCategoriaInversionSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddCategoriaInversion={onOpenAddCategoriaInversion}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditCategoriaInversion(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewCategoriaInversion(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeleteCategoriaInversion(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
      ...
      {showForm ? 
      (
        <CategoriaInversionCreador
          showForm={showForm}
          CategoriaInversion={CategoriaInversionSeleccionado}
          accion={accion}
...
export default CategoriaInversion;
