...
import NivelConsumoCreador from './NivelConsumoCreador';
import {
  onGetColeccion,
  onDelete,
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip
                  title='Crear NivelConsumo'
                  onClick={onOpenAddNivelConsumo}
                >
                  <IconButton
                    sx={{
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddNivelConsumo: PropTypes.func.isRequired,
  handleOpenPopoverColumns: PropTypes.func.isRequired,
  queryFilter: PropTypes.func.isRequired,
  limpiarFiltros: PropTypes.func.isRequired,
  nivelFiltro: PropTypes.string.isRequired,
...
const NivelConsumo = (props) => {
  ...
  const [NivelConsumoSeleccionado, setNivelConsumoSeleccionado] = useState(0);

  const { rows, desde, hasta, ultima_pagina, total } = useSelector(
    (state) => state.nivelConsumo,
  );
  ...
  const [nivelFiltro, setAnioFiltro] = useState('');
  const debouncedName = useDebounce(nivelFiltro, 800);
  ...
  const [columnasMostradas, setColumnasMostradas] =
    useState(columnasMostradasInicial);
  ...
  useEffect(() => {
    if (!user || !props.route?.path) return;
    ...
    user.usuario.permisos.forEach((modulo) => {
      modulo.opciones.forEach((opcion) => {
        if (opcion.url === props.route.path) {
          tituloAux = opcion.nombre || '';
          urlAyudaAux = opcion.url_ayuda || '';
  ...
  useEffect(() => {
    dispatch(onGetColeccion({ page, rowsPerPage, nivelFiltro, orderByToSend }));
  ...
  const onOpenEditNivelConsumo = (id) => {
    setNivelConsumoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewNivelConsumo = (id) => {
    setNivelConsumoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteNivelConsumo = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar el nivel consumo ?',
  ...
  const onOpenAddNivelConsumo = () => {
    setNivelConsumoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      ...
      >
        {permisos.length > 0 && (
          <EnhancedTableToolbar
            ...
            onOpenAddNivelConsumo={onOpenAddNivelConsumo}
                        ...
                        >
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditNivelConsumo(row.id)}
                                sx={{
                                  '&:hover': {
                                    cursor: 'pointer',
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewNivelConsumo(row.id)}
                                sx={{
                                  '&:hover': {
                                    color: theme.palette.colorHovers,
                                    cursor: 'pointer',
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.eliminar' />}>
                              <DeleteIcon
                                onClick={() => onDeleteNivelConsumo(row.id)}
                                sx={{
                                  '&:hover': {
                                    color: theme.palette.colorHovers,
                                    cursor: 'pointer',
      ...
      {showForm ? (
        <NivelConsumoCreador
          showForm={showForm}
          NivelConsumo={NivelConsumoSeleccionado}
          accion={accion}
...
NivelConsumo.propTypes = {
  route: PropTypes.shape({
    path: PropTypes.string.isRequired,
  }),
};

export default NivelConsumo;