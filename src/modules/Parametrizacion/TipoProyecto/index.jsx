...
import TipoProyectoCreador from './TipoProyectoCreador';
import {onGetColeccion, onDelete} from '../../../@crema/redux/features/tipoProyecto/tiposProyectosSlice';
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear TipoProyecto' onClick={onOpenAddTipoProyecto}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddTipoProyecto: PropTypes.func.isRequired,
...
const TipoProyecto = (props) => {
  ...
  const [tipoProyectoSeleccionado, setTipoProyectoSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.tiposProyectos);
  ...
  const onOpenEditTipoProyecto = (id) => {
    setTipoProyectoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewTipoProyecto = (id) => {
    setTipoProyectoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteTipoProyecto = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar el Tipo de Proyecto ?',
  ...
  const onOpenAddTipoProyecto = () => {
    setTipoProyectoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
      ...
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddTipoProyecto={onOpenAddTipoProyecto}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditTipoProyecto(row.id)}
                                sx={{'&:hover': {
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewTipoProyecto(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeleteTipoProyecto(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
