...
import {onGetColeccionTipo, onDelete, onUploadDocumento as onUpload} from '../../../@crema/redux/features/proveedorDocumento/proveedorDocumentosSlice';
...
const cells = [
  ...
  },
  {
    ...
    value: (value, row) => 
    {
      if (!row?.id_proveedor) return value; // Evita errores si no existe `id_proveedor`
      return  (
        <a
          href={`${defaultConfig.API_URL}/descargarProveedor/${row?.id_proveedor}/${value}`}
          target="_blank"
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('CrearDoc') >= 0 && (
                <Tooltip title='Crear ProveedorDocumento' onClick={onOpenAddProveedorDocumento}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddProveedorDocumento: PropTypes.func.isRequired,
...
const ProveedorDocumento = (props) => {
  ...
  const [ProveedorDocumentoSeleccionado, setProveedorDocumentoSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.proveedoresDocumento);
  ...
  useEffect(() => {
    dispatch(onGetColeccionTipo({page, rowsPerPage, nombreFiltro, tipoLista, proveedor_id, orderByToSend}));
  ...
  useEffect(() => {
      dispatch(onShow(proveedor_id));    
  }, [dispatch, proveedor_id]);
  ...
  const onOpenEditProveedorDocumento = (id) => {
    setProveedorDocumentoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onDeleteProveedorDocumento = (id) => {
    Swal.fire({
      title: 'Confirmar',
      ...
      if (result.isConfirmed) {
        dispatch(onDelete({ id, proveedor_id, updateColeccion }))        
      }
    });
  };
  ...
  const onOpenAddProveedorDocumento = () => {
    setProveedorDocumentoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  const onUploadDocumento = async (idListaDocumento) => {
    ...
    input.onchange = async (e) => {
      ...
      formData.append("id_proveedor", proveedor_id); // ID de la compañía actual
      ...
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddProveedorDocumento={onOpenAddProveedorDocumento}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('ModificarDoc') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditProveedorDocumento(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('EliminarDoc') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeleteProveedorDocumento(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
