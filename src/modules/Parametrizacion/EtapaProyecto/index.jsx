...
import EtapaProyectoCreador from './EtapaProyectoCreador';
import {onGetColeccion, onDelete} from '../../../@crema/redux/features/etapaProyecto/etapaProyectoSlice';
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear EtapaProyecto' onClick={onOpenAddEtapaProyecto}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddEtapaProyecto: PropTypes.func.isRequired,
...
const EtapaProyecto = (props) => {
  ...
  const [EtapaProyectoSeleccionado, setEtapaProyectoSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.etapaProyecto);
  ...
  useEffect(() => {
    dispatch(onGetColeccion({page, rowsPerPage, nombreFiltro, idTipoProyecto, orderByToSend}));
  }, [dispatch, page, rowsPerPage, debouncedName,debouncedTipoProyecto, orderByToSend, showForm]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    setPage(1)
  }, [debouncedName, debouncedTipoProyecto, orderByToSend]);
  ...
  const onOpenEditEtapaProyecto = (id) => {
    setEtapaProyectoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewEtapaProyecto = (id) => {
    setEtapaProyectoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteEtapaProyecto = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar la etapa proyecto ?',
  ...
  const onOpenAddEtapaProyecto = () => {
    setEtapaProyectoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddEtapaProyecto={onOpenAddEtapaProyecto}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditEtapaProyecto(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewEtapaProyecto(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeleteEtapaProyecto(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
      ...
      {showForm ? 
      (
        <EtapaProyectoCreador
          showForm={showForm}
          EtapaProyecto={EtapaProyectoSeleccionado}
          accion={accion}
