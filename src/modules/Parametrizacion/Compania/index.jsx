...
import CompaniaCreador from './CompaniaCreador';
import {onGetColeccion, onDelete} from '../../../@crema/redux/features/Compania/companiasSlice';
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear Compania' onClick={onOpenAddCompania}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddCompania: PropTypes.func.isRequired,
...
const Compania = (props) => {
  ...
  const [CompaniaSeleccionado, setCompaniaSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.companias);
  ...
  const onOpenEditCompania = (id) => {
    setCompaniaSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewCompania = (id) => {
    setCompaniaSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteCompania = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar la Compañia ?',
  ...
  const onOpenAddCompania = () => {
    setCompaniaSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  const onGoDocumentos = (compania_id) => {
    navigate('/documentos-companias/' + compania_id);
  } 
      ...
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddCompania={onOpenAddCompania}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditCompania(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewCompania(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeleteCompania(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
