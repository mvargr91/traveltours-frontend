...
import {
  ...
  onCreate,
  resetCompaniaActual,
}  from '../../../../@crema/redux/features/Compania/companiasSlice';
import { onGetColeccionLigera as onGetCompaniaes } from '../../../../@crema/redux/features/ciudades/ciudadesSlice';
...
const CompaniaCreador = (props) => {
  const { Compania, handleOnClose, accion, updateColeccion, titulo } = props;
  ...
  const loading = useSelector((state) =>  state.companias.loading);
  const { coleccionLigera: ciudades } = useSelector((state) => state.ciudades);
  const initializeSelectedRow = () => {
    dispatch(resetCompaniaActual()); 
  };
  useEffect(() => {
  ...
  useEffect(() => {
    if ((accion === 'editar') | (accion === 'ver')) {
      dispatch(onShow(Compania));
    }
  }, [accion, dispatch, Compania]);

    useEffect(() => {
      dispatch(onGetCompaniaes());
    }, [dispatch]);
...
export default CompaniaCreador;
