...
import PrecioProyectadoCreador from './PrecioProyectadoCreador';
import {onGetColeccion, onDelete} from '../../../@crema/redux/features/precioProyectado/precioProyectadoSlice';
...
const cells = [
  ...
  },
  {
    id: 'precio_com_ppa',
    typeHead: 'numeric',
    label: 'comercializador PPA',
    value: (value) => CurrencyDecimal(value),
    align: 'right',
    mostrarInicio: true,
  },
  {
    id: 'precio_com_representado',
    typeHead: 'numeric',
    label: 'comercializador representado',
    value: (value) => CurrencyDecimal(value),
    align: 'right',
    mostrarInicio: true,
  },
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear PrecioProyectado' onClick={onOpenAddPrecioProyectado}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddPrecioProyectado: PropTypes.func.isRequired,
...
const PrecioProyectado = (props) => {
  ...
  const [PrecioProyectadoSeleccionado, setPrecioProyectadoSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.precioProyectado);
  ...
  const onOpenEditPrecioProyectado = (id) => {
    setPrecioProyectadoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewPrecioProyectado = (id) => {
    setPrecioProyectadoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeletePrecioProyectado = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar el precio proyectado ?',
  ...
  const onOpenAddPrecioProyectado = () => {
    setPrecioProyectadoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddPrecioProyectado={onOpenAddPrecioProyectado}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditPrecioProyectado(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewPrecioProyectado(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeletePrecioProyectado(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
      ...
      {showForm ? 
      (
        <PrecioProyectadoCreador
          showForm={showForm}
          PrecioProyectado={PrecioProyectadoSeleccionado}
          accion={accion}
...
export default PrecioProyectado;
