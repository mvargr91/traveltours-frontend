...
import BancoCreador from './BancoCreador';
import {onGetColeccion, onDelete} from '../../../@crema/redux/features/bancos/bancosSlice';
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear Banco' onClick={onOpenAddBanco}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddBanco: PropTypes.func.isRequired,
...
const Banco = (props) => {
  ...
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.bancos);
  ...
  const onOpenViewBanco = (id) => {
    setBancoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteBanco = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar el Banco ?',
  ...
  const onOpenAddBanco = () => {
    setBancoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditBanco(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewBanco(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
