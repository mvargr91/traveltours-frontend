...
import CompaniaDocumentoCreador from './DocumentoCompaniaCreador';
import {onShow}  from '../../../@crema/redux/features/Compania/companiasSlice';
import {onGetColeccionTipo, onDelete, onUploadDocumento as onUpload} from '../../../@crema/redux/features/companiaDocumento/companiaDocumentosSlice';
...
const cells = [
  ...
  },
  {
    ...
    value: (value, row) => 
    {
      if (!row?.id_compania) return value; // Evita errores si no existe `id_compania`
      return  (
        <a
          href={`${defaultConfig.API_URL}/descargar/${row?.id_compania}/${value}`}
          target="_blank"
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('CrearDoc') >= 0 && (
                <Tooltip title='Crear CompaniaDocumento' onClick={onOpenAddCompaniaDocumento}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddCompaniaDocumento: PropTypes.func.isRequired,
...
const CompaniaDocumento = (props) => {
  const theme = useTheme();
  const navigate = useNavigate();
  const { compania_id } = useParams();
  ...
  const [CompaniaDocumentoSeleccionado, setCompaniaDocumentoSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.companiasDocumento);
  const selectedRow = useSelector((state) =>  state.companias.CompaniaActual);  
  ...
  useEffect(() => {
    dispatch(onGetColeccionTipo({page, rowsPerPage, nombreFiltro, tipoLista, compania_id, orderByToSend}));
  ...
  const onOpenEditCompaniaDocumento = (id) => {
    setCompaniaDocumentoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onDeleteCompaniaDocumento = (id) => {
    Swal.fire({
      title: 'Confirmar',
      ...
      if (result.isConfirmed) {
        dispatch(onDelete({ id, compania_id, updateColeccion }))        
      }
    });
  };
  ...
  const onOpenAddCompaniaDocumento = () => {
    setCompaniaDocumentoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  const onUploadDocumento = async (idListaDocumento) => {
    ...
    input.type = "file";
    input.accept = ".pdf"; // Asegura que solo suban PDFs
    ...
    input.onchange = async (e) => {
      ...
      formData.append("id_compania", compania_id); // ID de la compañía actual
      formData.append("archivo", file);
      formData.append("estado", 1); // Estado activo por defecto
  ...
  return (
    <div className={classes.root}>
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddCompaniaDocumento={onOpenAddCompaniaDocumento}
                    ...
                    return (
                      ...
                      >
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('ModificarDoc') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditCompaniaDocumento(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('EliminarDoc') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeleteCompaniaDocumento(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
      ...
      {showForm ? 
      (
        <CompaniaDocumentoCreador
          showForm={showForm}
          CompaniaDocumento={CompaniaDocumentoSeleccionado}
          accion={accion}
...
export default CompaniaDocumento;
