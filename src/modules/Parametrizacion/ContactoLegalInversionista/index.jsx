...
import { TIPO_DOCUMENTO, TIPO_CONTACTOS , TIPO_CONTACTO_LEGAL, TIPO_CUENTA } from '../../../shared/constants/ListaValores';
import InversionistaContactoLegalCreador from './ContactoInversionistaCreador';
import {onGetColeccion, onDelete} from '../../../@crema/redux/features/inversionistaContactoLegal/inversionistaContactosLegalesSlice';
...
const cells = [
  ...
  {
    id: 'tipo_contacto_legal',
    typeHead: 'string',
    label: 'Tipo contacto',
     value: (value) => TIPO_CONTACTO_LEGAL.map((estado) => estado.id === value ? estado.nombre : ''),
    align: 'left',
    mostrarInicio: true,
  },
  {
  ...
  },
  {
    ...
    typeHead: 'string',
    label: 'Tipo cuenta',
    value: (value) => TIPO_CUENTA.map((estado) => estado.id === value ? estado.nombre : ''),
    align: 'left',
    mostrarInicio: false,
  },
  {
...
const EnhancedTableToolbar = (props) => {
  ...
  const {
    numSelected,
    titulo,
    onOpenAddInversionistaContactoLegal,
        ...
        <>
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('CrearContLegal') >= 0 && (
                <Tooltip title='Crear Contacto Legal' onClick={onOpenAddInversionistaContactoLegal}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddInversionistaContactoLegal: PropTypes.func.isRequired,
...
const InversionistaContactoLegal = (props) => {
  ...
  const [InversionistaContactoLegalSeleccionado, setInversionistaContactoLegalSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.inversionistasContactoLegal);
      ...
      user.usuario.permisos.forEach((modulo) => {
        modulo.opciones.forEach((opcion) => {
          if (opcion.url === '/inversionistas') {
            setTitulo(`${opcion.nombre} - Contactos Legales `);
  ...
  const onOpenEditInversionistaContactoLegal = (id) => {
    setInversionistaContactoLegalSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };

  const onOpenViewInversionistaContactoLegal  = (id) => {
    setInversionistaContactoLegalSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };
  ...
  const onDeleteInversionistaContactoLegal = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro que desea eliminar el contacto legal?',
      icon: 'warning',
  ...
  const onOpenAddInversionistaContactoLegal = () => {
    setInversionistaContactoLegalSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddInversionistaContactoLegal={onOpenAddInversionistaContactoLegal}
                    ...
                    return (
                      ...
                      >
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('ModificarContLegal') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditInversionistaContactoLegal(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('ListarContLegal') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewInversionistaContactoLegal(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                          ...
                          {permisos.indexOf('EliminarContLegal') >= 0 && (
                            <Tooltip
                              title={<IntlMessages id='boton.eliminar' />}
                            >
                              <DeleteIcon
                                onClick={() => onDeleteInversionistaContactoLegal(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
      ...
      {showForm ? 
      (
        <InversionistaContactoLegalCreador
          showForm={showForm}
          InversionistaContactoLegal={InversionistaContactoLegalSeleccionado}
          accion={accion}
          handleOnClose={handleOnClose}
          updateColeccion={updateColeccion}
          titulo={titulo}
          inversionista={selectedRow}
          inversionistaId={inversionista_id}
        />
      ) : (
        ''
      )}
...
InversionistaContactoLegal.propTypes = {
...
export default InversionistaContactoLegal;
