...
import ActividadProyectoCreador from './ActividadProyectoCreador';
import {onGetColeccion, onDelete } from '../../../@crema/redux/features/actividadProyecto/actividadProyectoSlice';
...
const cells = [
  ...
  {
    id: 'peso_porcentual_etapa',
    typeHead: 'numeric',
    label: 'Peso porcentual etapa',
    value: (value) => value,
    align: 'right',
    mostrarInicio: false,
  },
  {
    id: 'peso_porcentual_proyecto',
    typeHead: 'numeric',
    label: 'Peso porcentual proyecto',
    value: (value) => value,
    align: 'right',
    mostrarInicio: false,
  },
  {
    id: 'actividad_prerequisito',
    typeHead: 'string',
    label: 'Actividad prerequisito',
    value: (value) => value,
    align: 'left',
    mostrarInicio: false,
  },
        ...
        <>
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear ActividadProyecto' onClick={onOpenAddActividadProyecto}>
                  <IconButton
          ...
          <Box className={classes.contenedorFiltros}>          
            ...
            <MUIAutocomplete
              ...
              value={etapasProyectos.find((i) => i.nombre === idEtapaProyecto) || null}
              onChange={(_, option) => setIdEtapaProyecto(option?.nombre ?? null)}
              getOptionLabel={(opt) => opt?.nombre || ''}
            />  
            <Box display='grid'>
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddActividadProyecto: PropTypes.func.isRequired,
...
const ActividadProyecto = (props) => {
  ...
  const [ActividadProyectoSeleccionado, setActividadProyectoSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.actividadProyecto);
  ...
  const debouncedTipoProyecto = useDebounce(idTipoProyecto, 800);
  const debouncedEtapaProyecto = useDebounce(idEtapaProyecto, 800);
  ...
  useEffect(() => {
    dispatch(onGetColeccion({page, rowsPerPage, idTipoProyecto, idEtapaProyecto, orderByToSend}));
  }, [dispatch, page, rowsPerPage,debouncedTipoProyecto, debouncedEtapaProyecto, orderByToSend, showForm]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    setPage(1)
  }, [debouncedTipoProyecto, debouncedEtapaProyecto, orderByToSend]);
  ...
  const onOpenEditActividadProyecto = (id) => {
    setActividadProyectoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewActividadProyecto = (id) => {
    setActividadProyectoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteActividadProyecto = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar la actividad proyecto ?',
  ...
  const onOpenAddActividadProyecto = () => {
    setActividadProyectoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddActividadProyecto={onOpenAddActividadProyecto}
            ...
            setIdEtapaProyecto={setIdEtapaProyecto}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditActividadProyecto(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewActividadProyecto(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeleteActividadProyecto(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
      ...
      {showForm ? 
      (
        <ActividadProyectoCreador
          showForm={showForm}
          ActividadProyecto={ActividadProyectoSeleccionado}
          accion={accion}
...
export default ActividadProyecto;
