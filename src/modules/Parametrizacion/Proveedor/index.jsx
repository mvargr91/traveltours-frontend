...
import ProveedorCreador from './ProveedorCreador';
import {onGetColeccion, onDelete} from '../../../@crema/redux/features/Proveedor/proveedoresSlice';
...
import { TIPO_DOCUMENTO, DATO_TIPO_PERSONA, TIPO_CUENTA} from '../../../shared/constants/ListaValores';
...
const cells = [
  ...
  },
  {
    ...
    typeHead: 'string',
    label: 'Tipo persona',
    value: (value) => DATO_TIPO_PERSONA.map((estado) => estado.value === value ? estado.label : ''),
    align: 'left',
    mostrarInicio: false,
  },
  {
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear Proveedor' onClick={onOpenAddProveedor}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddProveedor: PropTypes.func.isRequired,
...
const Proveedor = (props) => {
  ...
  const [ProveedorSeleccionado, setProveedorSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.proveedores);
  ...
  const onOpenViewProveedor = (id) => {
    setProveedorSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteProveedor = (id) => {
    Swal.fire({
      title: 'Confirmar',
  ...
  const onGoDocumentos = (proveedor_id) => {
    navigate('/documentos-proveedores/' + proveedor_id);
  } 
      ...
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddProveedor={onOpenAddProveedor}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditProveedor(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewProveedor(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeleteProveedor(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
