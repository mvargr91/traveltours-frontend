...
import GestorCreador from './GestorCreador';
import {onGetColeccion, onDelete} from '../../../@crema/redux/features/gestor/gestoresSlice';
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear Gestor' onClick={onOpenAddGestor}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddGestor: PropTypes.func.isRequired,
...
const Gestor = (props) => {
  ...
  const [GestorSeleccionado, setGestorSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.gestores);
  ...
  const onOpenEditGestor = (id) => {
    setGestorSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewGestor = (id) => {
    setGestorSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteGestor = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar el Gestor ?',
  ...
  const onOpenAddGestor = () => {
    setGestorSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  const onGoDocumentos = (gestor_id) => {
    navigate('/documentos-gestores/' + gestor_id);
  } 
      ...
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddGestor={onOpenAddGestor}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditGestor(row.id)}
                                sx={{'&:hover': {
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewGestor(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
