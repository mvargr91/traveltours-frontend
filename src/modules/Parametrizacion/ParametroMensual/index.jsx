...
import ParametroMensualCreador from './ParametroMensualCreador';
import {onGetColeccion, onDelete} from '../../../@crema/redux/features/parametroMensual/parametroMensualSlice';
...
import { CurrencyDecimal } from '../../../shared/hooks/formatCurrency';
...
const cells = [
  ...
  {
    id: 'valor_energia_bolsa',
    typeHead: 'numeric',
    label: 'Precio bolsa',
    value: (value) => CurrencyDecimal(value),
    align: 'right',
    mostrarInicio: true,
  },
  {
    id: 'valor_energia_mercado',
    typeHead: 'numeric',
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear ParametroMensual' onClick={onOpenAddParametroMensual}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddParametroMensual: PropTypes.func.isRequired,
  handleOpenPopoverColumns: PropTypes.func.isRequired,
  queryFilter: PropTypes.func.isRequired,
  limpiarFiltros: PropTypes.func.isRequired,
  anioFiltro: PropTypes.string.isRequired,
...
const ParametroMensual = (props) => {
  ...
  const [ParametroMensualSeleccionado, setParametroMensualSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.parametroMensual);
  ...
  const debouncedName = useDebounce(anioFiltro, 800);
  ...
  useEffect(() => {
    dispatch(onGetColeccion({page, rowsPerPage, anioFiltro, orderByToSend}));
  ...
  const onOpenEditParametroMensual = (id) => {
    setParametroMensualSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewParametroMensual = (id) => {
    setParametroMensualSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteParametroMensual = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar el parámetro mensual ?',
  ...
  const onOpenAddParametroMensual = () => {
    setParametroMensualSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddParametroMensual={onOpenAddParametroMensual}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditParametroMensual(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewParametroMensual(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeleteParametroMensual(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
      ...
      {showForm ? 
      (
        <ParametroMensualCreador
          showForm={showForm}
          ParametroMensual={ParametroMensualSeleccionado}
          accion={accion}
...
export default ParametroMensual;
