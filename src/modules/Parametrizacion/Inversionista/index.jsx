...
import InversionistaCreador from './InversionistaCreador';
import {onGetColeccion, onDelete} from '../../../@crema/redux/features/inversionista/inversionistasSlice';
...
import { TIPO_DOCUMENTO, TIPO_CUENTA, DATO_BOOLEAN_RADIO, DATO_BOOLEAN } from '../../../shared/constants/ListaValores';
...
const cells = [
  ...
  {
    id: 'porcentaje_ret_fuente_rendimientos',
    typeHead: 'numeric',
    label: '% Ret. fuente',
    value: (value) => value,
    align: 'right',
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear Inversionista' onClick={onOpenAddInversionista}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddInversionista: PropTypes.func.isRequired,
...
const Inversionista = (props) => {
  ...
  const [InversionistaSeleccionado, setInversionistaSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.inversionistas);
  ...
  const onOpenEditInversionista = (id) => {
    setInversionistaSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewInversionista = (id) => {
    setInversionistaSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteInversionista = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar el inversionista?',
  ...
  const onOpenAddInversionista = () => {
    setInversionistaSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  const onGoDocumentos = (inversionista_id) => {
    navigate('/documentos-inversionistas/' + inversionista_id);
  } 

  const onGoContactos = (inversionista_id) => {
    navigate('/contactos-inversionistas/' + inversionista_id);
  } 

  const onGoContactosLegales = (inversionista_id) => {
    navigate('/contactos-legales-inversionistas/' + inversionista_id);
  } 
      ...
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddInversionista={onOpenAddInversionista}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditInversionista(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewInversionista(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeleteInversionista(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                              ...
                              <Tooltip title={'Contactos'}>
                                <Box style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                                  <SupervisorAccountIcon
                                    ...
                                    color: theme.palette.colorFiltro,}}
                                    onClick={() => onGoContactos(row.id)}
                                    className={`${classes.generalIcons}`}/>
                                </Box>
                              </Tooltip>
                            </StyledTableCell>
                            <StyledTableCell align='center' className={classes.cell}>
                              <Tooltip title={'Contactos Leglaes'}>
                                <Box style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                                  <SchoolIcon
                                    sx={{'&:hover': {
                                      color: theme.palette.colorHovers,
                                    ...
                                    color: theme.palette.grayBottoms,}}
                                    onClick={() => onGoContactosLegales(row.id)}
                                    className={`${classes.generalIcons}`}/>
                                </Box>
                              </Tooltip>
                            </StyledTableCell>
