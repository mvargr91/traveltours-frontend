...
import ConceptoProyectoCreador from './ConceptoProyectoCreador';
import { onGetColeccion, onDelete } from '../../../@crema/redux/features/conceptoProyecto/conceptoProyectoSlice';
...
import { DATO_BOOLEAN_RADIO, TIPO_LISTA_RADIO, TIPO_CONCEPTO, VALOR_BASE, OPERADORES, TIPOS_VALOR_CONCEPTO} from '../../../shared/constants/ListaValores';
...
const cells = [
  ...
  {
    id: 'indicativo_tipo_concepto',
    typeHead: 'string',
    label: 'Tipo Concepto',
    value: (value) => TIPO_CONCEPTO.map((estado) => estado.id === value ? estado.nombre : ''),
    align: 'left',
    mostrarInicio: true,
  },
  {
    id: 'indicativo_tipo_valor',
    typeHead: 'string',
    label: 'Tipo valor',
    value: (value) => TIPOS_VALOR_CONCEPTO.map((estado) => estado.id === value ? estado.nombre : ''),
    align: 'left',
    mostrarInicio: false,
  },
    {
    id: 'indicativo_tipo_linea',
    typeHead: 'string',
    label: 'Tipo línea',
    value: (value) => TIPO_LISTA_RADIO.map((estado) => estado.value === value ? estado.label : ''),
    align: 'left',
    mostrarInicio: true,
  },
  {
    id: 'indicativo_presentacion_cons',
    typeHead: 'string',
    label: 'Presentacion consulta',
    value: (value) => DATO_BOOLEAN_RADIO.map((estado) => estado.value === value ? estado.label : ''),
    align: 'left',
    mostrarInicio: false,
  },
  {
    id: 'indicativo_permite_copia',
    typeHead: 'string',
    label: 'Copia',
    value: (value) => DATO_BOOLEAN_RADIO.map((estado) => estado.value === value ? estado.label : ''),
    align: 'left',
  ...
  {
    id: 'indicativo_concepto_calculado',
    typeHead: 'string',
    label: 'Calculado',
    value: (value) => DATO_BOOLEAN_RADIO.map((estado) => estado.value === value ? estado.label : ''),
    align: 'left',
  ...
  {
    id: 'indicativo_concepto_editable',
    typeHead: 'string',
    label: 'Editable',
    value: (value) => DATO_BOOLEAN_RADIO.map((estado) => estado.value === value ? estado.label: ''),
    align: 'left',
  ...
  {
    id: 'indicativo_valor_base',
    typeHead: 'string',
    label: 'Valor base',
    value: (value) => VALOR_BASE.map((estado) => estado.id === value ? estado.nombre: ''),
    align: 'left',
    mostrarInicio: false,
  },
  {
    id: 'indicativo_operador',
    typeHead: 'string',
    label: 'Operador',
    value: (value) => OPERADORES.map((estado) => estado.id === value ? estado.nombre: ''),
    align: 'left',
    mostrarInicio: false,
  },
  {
    id: 'porcentaje_proy',
    typeHead: 'string',
    label: 'Porcentaje Proyección',
    value: (value) => value ? value + ' %' : '',
    align: 'left',
    mostrarInicio: false,
  },
  {
    id: 'parametro_referencia_proy',
    typeHead: 'string',
    label: 'Parametro referencia Proyección',
    value: (value) => value,
    align: 'left',
    mostrarInicio: false,
  },
  {
    id: 'indicativo_valor_base_proy',
    typeHead: 'string',
    label: 'Valor base Proyección',
    value: (value) => VALOR_BASE.map((estado) => estado.id === value ? estado.nombre: ''),
    align: 'left',
    mostrarInicio: false,
  },
  {
    id: 'indicativo_operador_proy',
    typeHead: 'string',
    label: 'Operador Proyección',
    value: (value) => OPERADORES.map((estado) => estado.id === value ? estado.nombre: ''),
    align: 'left',
    mostrarInicio: false,
  },
  {
    id: 'numero_anio_inicial_proy',
    typeHead: 'string',
    label: 'Número años incial proyección',
    value: (value) => OPERADORES.map((estado) => estado.id === value ? estado.nombre: ''),
    align: 'left',
    mostrarInicio: false,
  },
  {
    id: 'numero_anios_proy',
    typeHead: 'string',
    label: 'Números años proyección',
    value: (value) => OPERADORES.map((estado) => estado.id === value ? estado.nombre: ''),
    align: 'left',
    mostrarInicio: false,
  },
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear ConceptoProyecto' onClick={onOpenAddConceptoProyecto}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddConceptoProyecto: PropTypes.func.isRequired,
...
const ConceptoProyecto = (props) => {
  ...
  const [idEtapaProyecto, setIdEtapaProyecto]   = useState(null);
  const [accion, setAccion] = useState('ver');
  const [ConceptoProyectoSeleccionado, setConceptoProyectoSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.conceptoProyecto);
  ...
  const onOpenEditConceptoProyecto = (id) => {
    setConceptoProyectoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewConceptoProyecto = (id) => {
    setConceptoProyectoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteConceptoProyecto = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar el concepto proyecto ?',
  ...
  const onOpenAddConceptoProyecto = () => {
    setConceptoProyectoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddConceptoProyecto={onOpenAddConceptoProyecto}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditConceptoProyecto(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewConceptoProyecto(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeleteConceptoProyecto(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
      ...
      {showForm ? 
      (
        <ConceptoProyectoCreador
          showForm={showForm}
          ConceptoProyecto={ConceptoProyectoSeleccionado}
          accion={accion}
...
export default ConceptoProyecto;
