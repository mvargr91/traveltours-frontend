...
import CondicionesPlazoCreador from './CondicionesPlazoCreador';
import {onGetColeccion, onDelete} from '../../../@crema/redux/features/condicionesPlazo/condicionesPlazoSlice';
...
const cells = [
  ...
  },
  {
    ...
    typeHead: 'numeric',
    label: 'Tasa interes inversión',
    value: (value) => value,
    align: 'right',
    mostrarInicio: true,
  },
  {
    id: 'porcentaje_inversion_externa',
    typeHead: 'numeric',
    label: '% Inversión externa',
    value: (value) => value,
    align: 'right',
    mostrarInicio: true,
  },
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear CondicionesPlazo' onClick={onOpenAddCondicionesPlazo}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddCondicionesPlazo: PropTypes.func.isRequired,
...
const CondicionesPlazo = (props) => {
  ...
  const [CondicionesPlazoSeleccionado, setCondicionesPlazoSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.condicionesPlazo);
  ...
  const onOpenEditCondicionesPlazo = (id) => {
    setCondicionesPlazoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewCondicionesPlazo = (id) => {
    setCondicionesPlazoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteCondicionesPlazo = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar la condición plazo ?',
  ...
  const onOpenAddCondicionesPlazo = () => {
    setCondicionesPlazoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddCondicionesPlazo={onOpenAddCondicionesPlazo}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditCondicionesPlazo(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewCondicionesPlazo(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                          ...
                          {permisos.indexOf('Eliminar') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeleteCondicionesPlazo(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
      ...
      {showForm ? 
      (
        <CondicionesPlazoCreador
          showForm={showForm}
          CondicionesPlazo={CondicionesPlazoSeleccionado}
          accion={accion}
...
export default CondicionesPlazo;
