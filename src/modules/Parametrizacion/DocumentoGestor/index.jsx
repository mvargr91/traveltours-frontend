...
import GestorDocumentoCreador from './DocumentoGestorCreador';
import {
  onShow,
}  from '../../../@crema/redux/features/gestor/gestoresSlice';
import {onGetColeccionTipo, onDelete, onUploadDocumento as onUpload} from '../../../@crema/redux/features/gestorDocumento/gestorDocumentosSlice';
...
const cells = [
  ...
  },
  {
    ...
    value: (value, row) => 
    {
      if (!row?.id_gestor) return value; // Evita errores si no existe `id_gestor`
      return  (
        <a
          href={`${defaultConfig.API_URL}/descargarGestor/${row?.id_gestor}/${value}`}
          target="_blank"
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('CrearDoc') >= 0 && (
                <Tooltip title='Crear GestorDocumento' onClick={onOpenAddGestorDocumento}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddGestorDocumento: PropTypes.func.isRequired,
...
const GestorDocumento = (props) => {
  const theme = useTheme();
  const navigate = useNavigate();
  const { gestor_id } = useParams();
  ...
  const [GestorDocumentoSeleccionado, setGestorDocumentoSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.gestoresDocumento);
  ...
  useEffect(() => {
    dispatch(onGetColeccionTipo({page, rowsPerPage, nombreFiltro, tipoLista, gestor_id, orderByToSend}));
  ...
  const onOpenEditGestorDocumento = (id) => {
    setGestorDocumentoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onDeleteGestorDocumento = (id) => {
    Swal.fire({
      title: 'Confirmar',
      ...
      if (result.isConfirmed) {
        dispatch(onDelete({ id, gestor_id, updateColeccion }))        
      }
    });
  };
  ...
  const onOpenAddGestorDocumento = () => {
    setGestorDocumentoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  const onUploadDocumento = async (idListaDocumento) => {
    ...
    input.onchange = async (e) => {
      ...
      formData.append("id_gestor", gestor_id); // ID de la compañía actual
  ...
  return (
    <div className={classes.root}>
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddGestorDocumento={onOpenAddGestorDocumento}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('ModificarDoc') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditGestorDocumento(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('EliminarDoc') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeleteGestorDocumento(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
      ...
      {showForm ? 
      (
        <GestorDocumentoCreador
          showForm={showForm}
          GestorDocumento={GestorDocumentoSeleccionado}
          accion={accion}
...
export default GestorDocumento;
