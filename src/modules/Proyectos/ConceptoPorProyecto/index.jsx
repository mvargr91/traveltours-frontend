...
import ConceptoPorProyectoCreador from './ConceptoPorProyectoCreador';
import ConceptoPorProyectoValores from './ConceptoPorProyectoValores';
import { onGetColeccion } from '../../../@crema/redux/features/conceptoPorProyecto/conceptoPorProyectoSlice';
...
import { onGetColeccionLigera as onGetParametrosConstantes } from '../../../@crema/redux/features/parametroConstante/parametroConstanteSlice';
...
const useToolbarStyles = makeStyles((theme) => ({
  ...
  contenedorFiltros: {
    width: '90%',
    display: 'grid',
    gridTemplateColumns: '4fr 2fr 2fr 1fr',
    gap: '20px',
      ...
      {numSelected > 0 ? (
        ...
        <>
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                  <Tooltip title='Crear Conceptos por proyecto' onClick={onOpenAddConceptoPorProyecto}>
                    <IconButton
          ...
          <Box className={classes.contenedorFiltros}>          
              <MUIAutocomplete
                label="Proyecto"
                options={proyectos.filter((item) => item.estado_proyecto === 1)}
              ...
              <TextField
                ...
                onBlur={(e) => {
                  ...
                  if (Number.isNaN(n)) { setAnioFiltro(''); return; }
                  const clamped = Math.min(Math.max(n, 2000), 3000);
                  setAnioFiltro(String(clamped));
                }}
              ...
              <TextField
                ...
                onBlur={(e) => {
                  ...
                  if (v === '') return;
                  const n = parseInt(v, 10);
                  if (Number.isNaN(n)) { setMesFiltro(''); return; }
                  const clamped = Math.min(Math.max(n, 1), 12);
                  setMesFiltro(String(clamped));
                }}
                className={classes.inputFiltros}
                variant='standard'
                fullWidth
...
const ConceptoPorProyecto = (props) => {
  ...
  const [ConceptoPorProyectoSeleccionado, setConceptoPorProyectoSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.conceptoPorProyecto);
  ...
  const { coleccionLigera: parametrosConstantes  } = useSelector((state) => state.parametroConstantes);
  const [permisos, setPermisos] = useState('');
  const [titulo, setTitulo] = useState('');
  const [idProyecto, setIdProyecto]   = useState(null);
  const [anioFiltro, setAnioFiltro] = useState('');
  const [mesFiltro, setMesFiltro] = useState('');
  const debouncedAnio = useDebounce(anioFiltro, 800);
  const debouncedMes = useDebounce(mesFiltro, 800);
  const debouncedProyecto = useDebounce(idProyecto, 800);
  ...
  useEffect(() => {
    ...
    dispatch(onGetParametrosConstantes());
  }, [dispatch]);
  ...
  useEffect(() => {
    dispatch(onGetColeccion({ page, rowsPerPage, idProyecto, anioFiltro, mesFiltro, orderByToSend }));
  }, [dispatch, page, rowsPerPage, orderByToSend, debouncedAnio, debouncedMes, debouncedProyecto, showForm]);


  const updateColeccion = () => {
    setPage(1);
    dispatch(onGetColeccion({ page: 1, rowsPerPage,  idProyecto, anioFiltro, mesFiltro, orderByToSend }));
  };

  useEffect(() => {
    setPage(1)
  }, [orderByToSend, debouncedAnio, debouncedMes, debouncedProyecto]);
  ...
  const queryFilter = (e) => {
    ...
    if (name === 'anioFiltro') {
      ...
      if (val !== '') {
        ...
        // Limita dentro de 2000–3000 mientras escribes
        if (n < 2000 && val.length === 4) val = '2000';
        if (n > 3000) val = '3000';
      }
      setAnioFiltro(val);
      return;
    ...
    if (name === 'mesFiltro') {
      ...
      if (val !== '') {
        const n = parseInt(val, 10);
        // Limita dentro de 1–12 mientras escribes
        if (n < 1 && val.length === 2) val = '1';
        if (n > 12) val = '12';
      }
      setMesFiltro(val);
      return;
  ...
  const onOpenAddConceptoPorProyecto = () => {
    setConceptoPorProyectoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };

  const onOpenEditConceptoPorProyecto = (row) => {
      setConceptoPorProyectoSeleccionado(row);
      setAccion('editar');
      setShowForm(true);
  };
  ...
  const onOpenViewConceptoPorProyecto = (row) => {
    setShowForm(true); 
    setConceptoPorProyectoSeleccionado(row);
    setAccion('ver');
  };
  ...
  return (
    <div className={classes.root}>
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onGoBack={onGoBack}
            permisos={permisos}
            onOpenAddConceptoPorProyecto={onOpenAddConceptoPorProyecto}
          />
        )}
        {showTable && permisos ? (
          <Box 
                    ...
                    return (
                      ...
                      >
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Valores') >= 0 && (
                            <Tooltip title={'Valores'}>
                              <AttachMoneyIcon
                                onClick={() => onOpenEditConceptoPorProyecto(row)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {/* {permisos.indexOf('ListarAct') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewConceptoPorProyecto(row)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                        ...
                        {columnasMostradas.map((columna) => {
                          if (columna.mostrar) {
                            return (
                              <MyCell
                                useStyles={useStyles}
                                key={String(getRowKey(row)) + columna.id}
                                align={columna.align}
                                width={columna.width}
                            ...
                            return <th key={String(getRowKey(row)) + columna.id}></th>;
                          }
                        })}
                      </StyledTableRow>
                    );
      ...
      {showForm  && accion == 'crear' ? 
      (
        <ConceptoPorProyectoCreador
          showForm={showForm}
          ConceptoPorProyecto={ConceptoPorProyectoSeleccionado}
          accion={accion}
...
export default ConceptoPorProyecto;
