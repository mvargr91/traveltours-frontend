...
import {onGetColeccion, onDelete, onHead} from '../../../@crema/redux/features/actividadPorProyecto/actividadPorProyectoSlice';
    ...
    >
     ...
     <Box sx={{ mt: 1 }}>
       ...
       <RadioGroup
         row
         value={estadoFiltro}
         onChange={onCambioEstado}
         name="filtro-estado"
         fontSize={12}
       >
         <FormControlLabel value="todos" control={<Radio />} label="Todos" />
         <FormControlLabel value="PEN" control={<Radio />} label="Pendiente" />
       </RadioGroup>
     </Box>
    </Toolbar>
  );
...
const ActividadPorProyecto = (props) => {
      ...
      user.usuario.permisos.forEach((modulo) => {
        modulo.opciones.forEach((opcion) => {
          if (opcion.url === props.route.path) {
            setTitulo( 'Actividades Por ' + opcion.nombre);
  ...
  useEffect(() => {
    dispatch(onGetColeccion({ page, rowsPerPage, proyecto_id, estadoFiltro, orderByToSend }));
  }, [dispatch, page, rowsPerPage, orderByToSend, estadoFiltro]);


  const updateColeccion = () => {
    setPage(1);
    dispatch(onGetColeccion({ page: 1, rowsPerPage, proyecto_id, estadoFiltro, orderByToSend }));
  };
  ...
  // Traduce el código del estado a su nombre legible
  const getNombreEstado = (codigo) =>
    ESTADO_ACTIVIDADES_POR_PROYECTO.find(e => String(e.id) === String(codigo))?.nombre
    ?? (codigo || 'Pendiente');


  // ====== Helpers de validación de prerequisito ======
  const isBlank = (v) => v === null || v === undefined || String(v).trim() === '';

  /**
   * Valida si la actividad puede abrir el registro (modal),
   * según el prerequisito y su estado (debe ser TER).
   */
  const puedeAbrirRegistro = (row, todasLasFilas) => {  
    const prereqId = row?.id_actividad_prerequisito;

    // No tiene prerequisito
    if (prereqId === null || prereqId === undefined || String(prereqId).trim() === '' || Number(prereqId) === 0) {
      return true;
    }

    // Busca la fila del prerequisito dentro del dataset actual
    const prereq = (todasLasFilas || []).find(
      (r) => String(r.id_actividad_proyecto) === String(prereqId)
    );

    if (!prereq) {
      Swal.fire({
        title: 'Prerrequisito no encontrado',
        html: `
          La actividad seleccionada requiere completar antes la actividad con ID <b>#${prereqId}</b>,
          pero no se encuentra en el listado actual.
        `,
    ...
    // Si el prerequisito no está terminado (TER), bloquear y mostrar el ESTADO del prerequisito
    if (prereq.estado_actividad !== 'TER') {
        const nombreEstado = getNombreEstado(prereq.estado_actividad);

        Swal.fire({
          title: 'Prerrequisito sin terminar', 
          html: `
            Debes terminar primero la actividad prerrequisito:
            <div style="margin-top:8px; ">
              <div><b>Actividad:</b> ${prereq.nombre_actividad || '(Sin nombre)'}</div>
              <div><b>Estado:</b>
                <span style="
  ...
  // validación previa de prerequisito
  const onOpenEditActividadPorProyecto = (row) => {
    // Validar prerequisito antes de abrir
    if (!puedeAbrirRegistro(row, rows)) return;
                    ...
                    return (
                      ...
                      >
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('RegistrarAct') >= 0 && row.estado_actividad !== 'TER' && (
                            <Tooltip title={'Registrar'}>
                              <CheckIcon
                                onClick={() => onOpenEditActividadPorProyecto(row)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('ListarAct') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewActividadPorProyecto(row)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
