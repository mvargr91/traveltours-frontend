...
import ProyectoDocumentoCreador from './DocumentoProyectoCreador';
import {
  onShow,
}  from '../../../@crema/redux/features/Proyecto/proyectosSlice';
import {onGetColeccionTipo, onDelete, onUploadDocumento as onUpload} from '../../../@crema/redux/features/proyectoDocumento/proyectoDocumentosSlice';
...
const cells = [
  ...
  },
  {
    ...
    value: (value, row) => 
    {
      if (!row?.id_proyecto) return value; // Evita errores si no existe `id_proyecto`
      return  (
        <a
          href={`${defaultConfig.API_URL}/descargarProyecto/${row?.id_proyecto}/${value}`}
          target="_blank"
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('CrearDoc') >= 0 && (
                <Tooltip title='Crear ProyectoDocumento' onClick={onOpenAddProyectoDocumento}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddProyectoDocumento: PropTypes.func.isRequired,
...
const ProyectoDocumento = (props) => {
  ...
  const [ProyectoDocumentoSeleccionado, setProyectoDocumentoSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.proyectosDocumento);
  ...
  useEffect(() => {
    dispatch(onGetColeccionTipo({page, rowsPerPage, nombreFiltro, tipoLista, proyecto_id, orderByToSend}));
  ...
  const onOpenEditProyectoDocumento = (id) => {
    setProyectoDocumentoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onDeleteProyectoDocumento = (id) => {
    Swal.fire({
      title: 'Confirmar',
      ...
      if (result.isConfirmed) {
        dispatch(onDelete({ id, proyecto_id, updateColeccion }))        
      }
    });
  };
  ...
  const onOpenAddProyectoDocumento = () => {
    setProyectoDocumentoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  const onUploadDocumento = async (idListaDocumento) => {
    ...
    input.onchange = async (e) => {
      ...
      formData.append("id_proyecto", proyecto_id); // ID de la compañía actual
  ...
  return (
    <div className={classes.root}>
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddProyectoDocumento={onOpenAddProyectoDocumento}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('ModificarDoc') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditProyectoDocumento(row.id)}
                                sx={{'&:hover': {
                                  cursor: 'pointer',
                          ...
                          {permisos.indexOf('EliminarDoc') >= 0 && (
                            ...
                            >
                              <DeleteIcon
                                onClick={() => onDeleteProyectoDocumento(row.id)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
      ...
      {showForm ? 
      (
        <ProyectoDocumentoCreador
          showForm={showForm}
          ProyectoDocumento={ProyectoDocumentoSeleccionado}
          accion={accion}
...
export default ProyectoDocumento;
