...
const PlanInversionProyectoForm = (props) => {
  ...
  useEffect(() => {
    if (!Array.isArray(values.plan_pagos) || values.plan_pagos.length !== 100) {
      setFieldValue('plan_pagos', generarPlanPagos(100));
    }
  }, [values.plan_pagos, setFieldValue]);

  useEffect(() => {
    if (Object.keys(stateData).length !== 0 && !stateDataHandled) {
      setStateDataHandled(true);
    }
  }, [stateData, stateDataHandled]);
  ...
  }, [selectedRowInversionista, values.id_inversionista, setValues]);

  useEffect(() => {
    if (selectedRowProyecto && values.id_proyecto) {
  ...
  const planPagos = Array.isArray(values.plan_pagos)
    ? values.plan_pagos
    : generarPlanPagos(100);

  const totalPaginas = Math.ceil(planPagos.length / porPagina);
...
PlanInversionProyectoForm.propTypes = {
...
PlanInversionProyectoForm.defaultProps = {
  initialValues: {},
  values: {},
  inversionistas: [],
  ciudades: [],
...
export default PlanInversionProyectoForm;