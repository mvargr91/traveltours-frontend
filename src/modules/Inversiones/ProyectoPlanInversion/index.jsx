...
import {onGetColeccion, onDelete} from '../../../@crema/redux/features/proyectoPlanInversion/proyectoPlanInversionSlice';
...
import { TIPO_DOCUMENTO, TIPO_CUENTA, ESTADOS_PROYECTOS_PLAN_INVERSIONES, DATO_BOOLEAN } from '../../../shared/constants/ListaValores';
...
const EnhancedTableToolbar = (props) => {
  ...
  const {
    numSelected,
    titulo,
    onOpenAddPlanInversionProyecto,
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip title='Crear PlanInversionProyecto' onClick={onOpenAddPlanInversionProyecto}>
                  <IconButton
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddPlanInversionProyecto: PropTypes.func.isRequired,
...
const PlanInversionProyecto = (props) => {
  const theme = useTheme();
  const navigate = useNavigate();
  const [order, setOrder] = React.useState('asc');
  const [orderBy, setOrderBy] = React.useState('');
  const [orderByToSend, setOrderByToSend] = React.useState(
    'nombre_proyecto:desc',
  );
  ...
  const [PlanInversionProyectoSeleccionado, setPlanInversionProyectoSeleccionado] = useState(0);
  const { rows, desde, hasta, ultima_pagina, total } = useSelector((state) => state.proyectoPlanInversion);
  ...
  useEffect(() => {
    ...
    dispatch(onGetColeccion({page, rowsPerPage, nombreFiltro, idProyecto, orderByToSend}));
  }, [dispatch, page, rowsPerPage, debouncedName, idProyecto, orderByToSend]);
  ...
  useEffect(() => {
    setPage(1);
  }, [debouncedName, idProyecto, orderByToSend]);
  ...
  const onOpenEditPlanInversionProyecto = (row) => {
    navigate('/proyectos-plan-inversiones/editar/'+row.id_inversionista+'/'+row.id_proyecto);
  };

  const onOpenViewPlanInversionProyecto = (row) => {
    navigate('/proyectos-plan-inversiones/ver/'+row.id_inversionista+'/'+row.id_proyecto);
  };
  ...
  const onDeletePlanInversionProyecto = (row) => {
    console.log(row);
    const inversionista = row?.id_inversionista;
    const proyecto = row?.id_proyecto
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar el Plan Inversión Proyecto?',
      ...
      if (result.isConfirmed) {
        dispatch(onDelete({inversionista, proyecto, updateColeccion}));
      }
    });
  };

  const onOpenAddPlanInversionProyecto = () => {
    navigate('/proyectos-plan-inversiones/crear');
  };

  const handleOnClose = () => {
    setPlanInversionProyectoSeleccionado(0);
    setAccion('ver');
  };
      ...
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddPlanInversionProyecto={onOpenAddPlanInversionProyecto}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>
                          {permisos.indexOf('Modificar') >= 0 &&(
                            <Tooltip title={<IntlMessages id='boton.editar' />}>
                              <EditIcon
                                onClick={() => onOpenEditPlanInversionProyecto(row)}
                                sx={{'&:hover': {                                  
                          ...
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewPlanInversionProyecto(row)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
                          ...
                          {permisos.indexOf('Eliminar') >= 0 &&  (
                            <Tooltip
                              ...
                              <DeleteIcon
                                onClick={() => onDeletePlanInversionProyecto(row)}
                                sx={{'&:hover': {
                                  color: theme.palette.colorHovers,
                                  cursor: 'pointer',
                                },
...
PlanInversionProyecto.propTypes = {
...
export default PlanInversionProyecto;
