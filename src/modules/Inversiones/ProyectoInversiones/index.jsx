...
import { TIPO_DOCUMENTO, TIPO_CUENTA, DATO_BOOLEAN , TIPOS_VALOR, ESTADOS_INVERSIONES} from '../../../shared/constants/ListaValores';
...
EnhancedTableToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
  onOpenAddInversionProyecto: PropTypes.func.isRequired,
...
const InversionProyecto = (props) => {
  ...
  const [InversionProyectoSeleccionado, setInversionProyectoSeleccionado] = useState(0);
      ...
      user.usuario.permisos.forEach((modulo) => {
        modulo.opciones.forEach((opcion) => {
          if (opcion.url === '/inversiones') {
            setTitulo(opcion.nombre + ' - Proyectos');
  ...
  useEffect(() => {
    dispatch(onGetColeccion({page, rowsPerPage, inversion_id, nombreFiltro, asuntoFiltro, orderByToSend}));
  }, [dispatch, page, rowsPerPage, inversion_id, debouncedName, debouncedAsunto, orderByToSend, showForm]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const onOpenEditInversionProyecto = (id) => {
    setInversionProyectoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewInversionProyecto = (id) => {
    console.log(id);
    setInversionProyectoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  ...
  const onOpenAddInversionProyecto = () => {
    setInversionProyectoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper sx={{marginBottom: theme.spacing(2),}} className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddInversionProyecto={onOpenAddInversionProyecto}
                        ...
                        <StyledTableCell align='center' className={classes.acciones}>                          
                          {permisos.indexOf('Listar') >= 0 && (
                            <Tooltip title={<IntlMessages id='boton.ver' />}>
                              <VisibilityIcon
                                onClick={() => onOpenViewInversionProyecto(row.id_proyecto)}
                                sx={{'&:hover': {
      ...
      (
        <ProyectoCreador
          showForm={showForm}
          Proyecto={InversionProyectoSeleccionado}
          accion={accion}
...
export default InversionProyecto;
