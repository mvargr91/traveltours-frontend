...
const Inversion = (props) => {
  ...
  const { rows, desde, hasta, ultima_pagina, total, totalGeneral, totalRteFuente, totalNeto } = useSelector((state) => state.planDetallado);
  ...
  const [hayErrores, setHayErrores] = useState(true);
...
const manejarCambioAccion = (id, tipoAccion, tipoConcepto) => {
  ...
  const validarAccion = (id, acciones, row, accionesGlobales) => {
    ...
    if (acciones.pago && acciones.rei) {
      return 'No puede seleccionarse accion pagar y reinvertir al mismo tiempo.';
    }
    ...
    if (acciones.rei && !(row.tipo === 'I' || row.tipo === 'K')) {
     ...
     if (acciones.rei) {
      ...
      if (reinversionesActuales.length > 0) {
        const base = reinversionesActuales[0]; // ← Primera reinversión marcada como referencia

        if (base.id_cliente !== row.id_cliente) {
          return ' Solo se permite registrar reinversiones de un solo inversionista';
        }

        if (base.fecha !== row.fecha) {
          return 'Solo se permite registrar reinversiones de una sola fecha';
        }
      }
    }
...
useEffect(() => {
  const tieneErrores = Object.keys(erroresPorFila).length > 0;
  const ningunaAccionMarcada = Object.values(accionesSeleccionadasGlobal).length === 0;
  setHayErrores(tieneErrores || ningunaAccionMarcada);
}, [erroresPorFila, accionesSeleccionadasGlobal]);
...
>
  <Box sx={{ gridColumn: '1 / 5', px: 2 }} />
  <Box sx={{ textAlign: 'right', px: 0 }}>Totales:</Box>
  <Box sx={{ textAlign: 'right', px: 0 }}>{formatCurrency(totalGeneral)}</Box>
  <Box sx={{ textAlign: 'right', px: 3 }}>{formatCurrency(totalRteFuente)}</Box>
  <Box sx={{ textAlign: 'right', px: 0 }}>{formatCurrency(totalNeto)}</Box>
</Box>
...
>
  <Box sx={{ gridColumn: '1 / 5', px: 2 }} />
  <Box sx={{ textAlign: 'right', px: 0 }}>Total Seleccionado para pago:</Box>
  <Box sx={{ textAlign: 'right', px: 0 }}>{formatCurrency(totales.valor)}</Box>
  <Box sx={{ textAlign: 'right', px: 3 }}>{formatCurrency(totales.ret_fuente)}</Box>
  <Box sx={{ textAlign: 'right', px: 0 }}>{formatCurrency(totales.valor_a_pagar)}</Box>
</Box>
