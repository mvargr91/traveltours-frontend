...
import { onGuardarProgramacion, clearPlanDetallado, onGetPagoProgramados, onGuardarPagosProgramados } from '../../../@crema/redux/features/planDetallado/planDetalladoSlice';
...
const Pago = (props) => {
  ...
  const totalConfirmadoRef = useRef(0); // persistente
  ...
  useEffect(() => {    
      dispatch(onGetPagoProgramados({ page, rowsPerPage, orderByToSend }));    
...
const manejarCambioAccion = async (id) => {
  const stringId = String(id);
  const row = rowsVisibles.find((r) => `${r.id}` === stringId);

  if (!row) return;

  const resultado = await Swal.fire({
    title: 'Confirmar pago',
    text: `¿Desea confirmar el pago de ${formatCurrency(row.valor_total_pagar)}?`,
  ...
  try {
    const res = await dispatch(onGuardarPagosProgramados(row));
    console.log(res)
    if (res.meta.requestStatus === 'fulfilled') {
      totalConfirmadoRef.current += parseFloat(row.valor_total_pagar || 0);
      setTotalVisual(totalConfirmadoRef.current);
      ...
      // Eliminación visual sin afectar Redux
      setRowsVisibles((prevRows) => prevRows.filter((r) => `${r.id}` !== stringId));
    }
  } catch (error) {
            ...
            <TableContainer component={Paper}>
                      ...
                      >
                        <StyledTableCell>
                          <Tooltip title='Confirmar Pago'>
                            <Checkbox
                              checked={accionesSeleccionadas[`${row.id}`]?.pago || false}
                              onChange={() => manejarCambioAccion(`${row.id}`)}
                            />
                          </Tooltip>
                        </StyledTableCell>
             ...
             <Box sx={{ mt: 2 }}>
                ...
                <Box
                  sx={{
                    display: 'grid',
                    gridTemplateColumns: '4fr 1fr 1fr 1fr 1fr 1fr 3fr 3fr',
                    alignItems: 'center',
                    px: 2,
                    py: 1,
                ...
                >
                  <Box sx={{ gridColumn: '1 / 8' }}></Box>
                  {/* <Box sx={{ textAlign: 'right' }}>Total Valor: {formatCurrency(totalGeneral)}</Box> */}
                  <Box sx={{ textAlign: 'right' }}>Total a Pagar: {formatCurrency(totalNeto)}</Box>
                </Box>
              </Box>
            <Box className={classes.paginacion}
             sx={{
