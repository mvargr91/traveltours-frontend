...
const MyAutocompleteTasa = (props) => {
  ...
  useEffect(() => {
    if (field.value !== '') {
      ...
      if (option) {
        setInputValue(option.codigo_tasa);
  ...
  return (
    <Autocomplete
      ...
      getOptionLabel={(option) => option?.codigo_tasa || ''}
      options={(props.options || []).filter(option => 
        option.codigo_tasa.toLowerCase().includes(inputValue.toLowerCase())
      )}
      renderInput={(params) => (
        <TextField
          {...params}
...
export default MyAutocompleteTasa;
